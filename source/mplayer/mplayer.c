...
// cache2:
#ifdef GEKKO
int stream_cache_size=1*1024; // normally 8 MB
#else
...
//find_prob = 0; // = no fps drops, when using subs
        ...
        *blit_frame    = (decoded_frame && filter_video(sh_video, decoded_frame,
		...
		//find_prob = 1;
		// SuSo: disable frame gen
        int res = 0;//generate_video_frame(sh_video, mpctx->d_video);
        if (!res)
            return -1;
...
#ifdef GEKKO
m_config_set_option(mconfig,"vo","null");
...
//m_config_set_option(mconfig,"lavdopts","fast=1");

//use ffmpeg to demux, fixes audio not playing in SFD videos

//m_config_set_option(mconfig,"lavcopts","lavc=wmav1");
...
void wiiSync(int sec)
{
	// SuSo: FLAC seeking isn't accurate!
	//if(mpctx->demuxer->file_format == 17) // applies to MP3/WAV/FLAC
	//if(strncmp(mpctx->sh_audio->codec->name, "ffflac", 6) == 0)
	//	sec *= 40;
	//printf("SHOWME: %s,,", mpctx->sh_audio->codec->name);
...
void wiiSetKaraoke()
{
	// Need to clear prevoious filter?
	//wiiSetVolNorm0();
...
void wiiSetRadio()
{
	...
	cmd->name=strdup("af_switch");
	mp_input_queue_cmd(cmd);
	cmd->args[0].v.s = strdup("equalizer=-12:-8:-6:-2:3:5:3:0:-8:-12");
	
	/*
		Yes, the parameters for this filter were taken from Google's AI mode.
		
		-12 (0-100 Hz)    : Cuts deep bass.
		-8  (100-200 Hz)  : Reduces lower-mids.
		-6  (200-400 Hz)  : Cuts muddy frequencies.
		-2  (400-800 Hz)  : Slight cut to warm mids.
		+3  (800-1600 Hz) : Boosts core mids for voice presence.
		+5  (1.6-3.2 kHz) : Strong boost to emphasize the "telephone" or radio vocal range.
		+3  (3.2-6.4 kHz) : Slight boost to higher vocal harmonics.
		 0  (6.4-12.8 kHz): No change.
		-8  (12.8-16 kHz) : Reduces crispness.
		-12 (16-20 kHz)   : Cuts high-end treble.
	*/
}
...
void wiiSetMono()
{
	...
	cmd->name=strdup("af_switch");
	mp_input_queue_cmd(cmd);
	cmd->args[0].v.s = strdup("extrastereo=0.0");
}
...
void wiiSetProperty(int command, float value)
{
	...
	switch(command)
	{
		...
		case MP_CMD_SWITCH_RATIO:
			cmd->name = strdup("switch_ratio"); /*force_load_font = 1;*/ break;
