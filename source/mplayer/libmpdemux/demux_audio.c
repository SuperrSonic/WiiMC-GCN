...
//	buf[0], buf[sizeof(buf)], 0);
		...
		for(int i=32; i < loop_limit; i++) { // start at pos 32 should speed up search
			...
			if(cur_bytes[0] == 'A' && cur_bytes[1] == 'P' && cur_bytes[2] == 'I' && cur_bytes[3] == 'C') {
				...
				if(val_apic_size > 1*1024*1024)
					val_apic_size = 1*1024*1024;
					//break;
	...
	//	pos_pic = (u8 *)memalign(32, 200*1024);
	//	stream_seek(s, 0x7F);
...
//    printf("wav: %X .. %X\n",(int)demuxer->movi_start,(int)demuxer->movi_end);
  ...
  case fLaC:
		...
		int val_apic_size = 0;
		if(found_ID3) {
		for(int i=32;i < (loop_limit + 0x320);i++) { // start at pos 32 should speed up search
			// skip if banner ss
			if(bannerSSactive || menuCurrent != 1)
				break;
...
static void demux_audio_seek(demuxer_t *demuxer,float rel_seek_secs,float audio_delay,int flags){
  ...
  // SuSo: quality seeking should always be enabled
  //if(priv->frmt == MP3 && hr_mp3_seek && !(flags & SEEK_FACTOR)) {
  if(priv->frmt == MP3 && !(flags & SEEK_FACTOR)) {
