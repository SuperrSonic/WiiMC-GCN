...
bool GuiTrigger::Left()
{
			...
			|| WPAD_StickX(0) < -PADCAL)
	{
		if(wpad->btns_d & (wiibtn | WPAD_CLASSIC_BUTTON_LEFT)
			|| pad.btns_d & PAD_BUTTON_LEFT || cpad.data.down & CTR_BUTTON_LEFT ||
												cpad.data.down & CTR_STICK_LEFT)
		{
			prevv[chan] = gettime();
		...
		if(diff_usec(prevv[chan], now[chan]) > delay[chan])
		{
			prevv[chan] = now[chan];
