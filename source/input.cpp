...
#else
	if(down & PAD_BUTTON_B || userInput[0].cpad.data.down & CTR_BUTTON_B) {
		osdLevel ^= 1;
	} else if(AutobootExit && (down & PAD_BUTTON_START ||
			userInput[0].cpad.data.down & CTR_BUTTON_START)) {
		VIDEO_SetBlack (TRUE);
		ExitRequested = true;
	} else if(ExitRequested || down & PAD_BUTTON_START ||
			userInput[0].cpad.data.down & CTR_BUTTON_START) {
		if(wiiIsPaused())
			wiiPause();
		wiiGotoGui();
...
#else
		if(down & PAD_BUTTON_A || userInput[0].cpad.data.down & CTR_BUTTON_A)
#endif
		{
