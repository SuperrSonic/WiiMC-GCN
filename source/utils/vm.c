...
//	printf("VMSize %08x MEMSize %08x VM_Base %p pmap_max %u\n", VMSize, MEMSize, VM_Base, pmap_max);
	...
	MEM_Base = (vm_page*)SYS_AllocArenaMemHi(MEMSize, PAGE_SIZE);
...
int vm_dsi_handler(u32 DSISR, u32 DAR)
{
	...
	if (phys_map[phys_index].dirty)
	{
		...
		DCFlushRange(MEM_Base+phys_index, PAGE_SIZE*pages_to_flush);
		ARQ_PostRequest(&vm_request, VM_VSID, ARQ_MRAMTOARAM, ARQ_PRIO_LO, flush_v_index*PAGE_SIZE, (u32)MEM_Base+phys_index, PAGE_SIZE*pages_to_flush);
//		printf("VM page %d was purged (%d)\n", phys_map[phys_index].page_index, pages_to_flush);
	...
	{
		ARQ_PostRequest(&vm_request, VM_VSID, ARQ_ARAMTOMRAM, ARQ_PRIO_HI, virt_index*PAGE_SIZE, (u32)MEM_Base+phys_index, PAGE_SIZE);
		DCInvalidateRange(MEM_Base+phys_index, PAGE_SIZE);
