...
static u8 * RawTo2x2RGB(u8 *src, u32 width, u32 height, u32 rowsize, int * dstWidth, int * dstHeight, u8 *dstPtr)
{
	u8 *dst;
	int x, y;
	u8 *pixel;
...
u8 * DecodeJPEG(const u8 *src, u32 len, int *width, int *height, u8 *dstPtr)
{
	...
	//if(WiiSettings.jpegQuality && output_width > 188 && output_width != 768 && output_height != 480 && !bannerSSactive) {
	if(WiiSettings.jpegQuality && output_width > 188 && !bannerSSactive) {
		//For slow but smooth art setting
		...
		} else if(WiiSettings.screensaverArt != ART_FULL) {
			output_width = 188;
			output_height = output_width * ar;
		}
	}
...
//	}
	...
	// Photo Channel style
		...
		if(output_width > limit || output_height > limit)
		{
			cinfo.scale_num = 1;
			cinfo.scale_denom = 1;
			while(output_width > limit || output_height > limit)
			{
				output_width /= 2;
				output_height /= 2;
				cinfo.scale_denom *= 2;
	...
	// See gui_imagedata to change format id
	//u8 *dst = RawTo4x4RGBA(tmpData, output_width, output_height, stuffer.stride, width, height, dstPtr);
	u8 *dst = RawTo2x2RGB(tmpData, output_width, output_height, stuffer.stride, width, height, dstPtr);
